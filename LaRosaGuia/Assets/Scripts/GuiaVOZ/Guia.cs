...
namespace LuckArkman.XR.Main
{
    public class Guia : MonoBehaviour
    {
        ...
        [Tooltip("Áudio de alerta do sistema tocado quando a conexão é estabelecida.")]
        public AudioClip alertAudioClip;

        [Tooltip("Áudio de introdução / tutorial tocado após o alerta de conexão.")]
        public AudioClip introducaoAudioClip;

        [SerializeField]
        private AudioSource _systemAudioSource;

        private bool _sistemaAudioAtivo = false;
        private bool _sistemaAudioAgendado = false;
        private Coroutine _systemAudioCoroutine = null;

        public bool EstaTocandoAudioDeSistema => _sistemaAudioAtivo || _sistemaAudioAgendado ||
            (_systemAudioSource != null && _systemAudioSource.isPlaying) ||
        ...
        // Estrutura para Ollama (LLaMA via Docker) - CORRIGIDO AQUI
        [global::System.Serializable] private class OllamaRequest { public string model; public string prompt; public bool stream; }
        [global::System.Serializable] private class OllamaResponse { public string response; public bool done; }
        ...
        private void Start()
        {
            ...
            if (piperOnnxTTS == null)
                piperOnnxTTS = FindFirstObjectByType<PiperOnnxTTS>();
            ...
            if (piperOnnxTTS != null)
            {
                ...
                Debug.Log("[Guia ✅] PiperOnnxTTS on-device detectado. TTS offline ativado, servidor HTTP desativado.");
            }
        ...
        /// </summary>
        public void IniciarSequenciaDeAudioDeSistema()
        {
            if (_sistemaAudioAtivo || _sistemaAudioAgendado || _systemAudioCoroutine != null) return;
            _sistemaAudioAgendado = true;
            _systemAudioCoroutine = StartCoroutine(EscalonarAudioDeSistema());
        }

        private IEnumerator EscalonarAudioDeSistema()
        {
            while (EstaAudioEmExecucao())
            {
                yield return null;
            }

            _sistemaAudioAgendado = false;
            _sistemaAudioAtivo = true;
            yield return StartCoroutine(TocarSequenciaDeAudioDeSistema());
            _sistemaAudioAtivo = false;
            _systemAudioCoroutine = null;
        }

        private bool EstaAudioEmExecucao()
        {
            if (_ttsOcupado) return true;
            if (spatialAudio != null && spatialAudio.EstaReproduziindo) return true;
            if (_systemAudioSource != null && _systemAudioSource.isPlaying) return true;
            if (voiceAudioSource != null && voiceAudioSource.isPlaying) return true;
            return false;
        }

        private AudioSource ObterOuCriarSystemAudioSource()
        {
            if (_systemAudioSource == null)
            {
                if (voiceAudioSource != null)
                {
                    _systemAudioSource = voiceAudioSource;
                }
                else
                {
                    _systemAudioSource = GetComponent<AudioSource>();
                }

                if (_systemAudioSource == null)
                {
                    _systemAudioSource = gameObject.AddComponent<AudioSource>();
                }

                _systemAudioSource.playOnAwake = false;
            }
            return _systemAudioSource;
        }

        private IEnumerator TocarSequenciaDeAudioDeSistema()
        {
            _sistemaAudioAtivo = true;
            AudioSource audioSource = ObterOuCriarSystemAudioSource();

            if (introducaoAudioClip != null)
            {
                audioSource.PlayOneShot(introducaoAudioClip);
                yield return new WaitForSeconds(introducaoAudioClip.length);
            }
            ...
            if (alertAudioClip != null)
            {
                ...
                audioSource.Play();
                yield return new WaitForSeconds(alertAudioClip.length);
            }
        ...
        public void ExecutarComando(EstadoInstrucao comandoDecidido, int passosObjeto = 0, string descricaoAmbiente = "", string motivoSemantico = "")
        {
            ...
            if (_sistemaAudioAtivo || _sistemaAudioAgendado || _ttsOcupado || (spatialAudio != null && spatialAudio.EstaReproduziindo)) return;
