...
namespace LuckArkman.XR.AI
{
    ...
    struct MidasMaxDepthJob : IJob
    {
        ...
        [WriteOnly] public NativeArray<float> maxDepthOut; 
    ...
    struct MidasZoneJob : IJobParallelFor
    {
        ...
        public int inverterProfundidade; // 0=MiDaS(1-norm), 1=DepthAnything(norm)
        ...
        public void Execute(int y)
        {
            ...
            for (int x = 0; x < tensorSize; x++)
            {
                ...
                // ====================================================
                // ADAPTAÇÃO HÍBRIDA MULTITHREAD:
                // Se inverterProfundidade = 1 (DepthAnything), Perigo é o valor direto.
                // Se inverterProfundidade = 0 (MiDaS), Perigo é 1 - valor.
                // ====================================================
                float perigo = (inverterProfundidade == 1) ? normalizado : (1f - normalizado);
    ...
    public class MonocularVisionManager : MonoBehaviour
    {
        public enum ModeloIA { MiDaS_V2, Depth_Anything_V2 }

        [Header("Configuração Híbrida de IA")]
        public ModelAsset modelAsset;
        
        [Tooltip("Escolha a IA para o Unity ajustar a resolução (256 para MiDaS, 518 para Depth Anything).")]
        public ModeloIA modeloAtivo = ModeloIA.Depth_Anything_V2;

        [Tooltip("Ative caso a IA esteja dizendo que tudo está perto quando na verdade está longe.")]
        public bool inverterMapaProfundidade = true;
        ...
        private int currentTensorSize = 256;
        ...
        private void Start()
        {
            if (modelAsset != null)
            {
                // Define resolução de forma inteligente
                currentTensorSize = (modeloAtivo == ModeloIA.Depth_Anything_V2) ? 518 : 256;

                runtimeModel = ModelLoader.Load(modelAsset);
                engineWorker = new Worker(runtimeModel, BackendType.GPUCompute);
                Debug.Log($"[Vision Manager] Motor carregado: {modeloAtivo} | Tensor: {currentTensorSize}x{currentTensorSize}");
            }
        }
        ...
        public MidasResult ExecuteInference(Texture2D cameraImage)
        {
            ...
            // 1. Tensor de entrada dinâmico
            TensorShape shape = new TensorShape(1, 3, currentTensorSize, currentTensorSize);
            ...
            // 3. Processa zonas com Jobs Burst (Multithreaded)
            ProcessarZonasDeRiscoJob(depthArray, ref result);

            // 4. Cálculo de velocidade
            ...
            {
                Debug.Log($"[{modeloAtivo}] Frente: {result.dangerScore:F1}/10 | Esq: {result.leftZoneDanger:F1}/10 | Dir: {result.rightZoneDanger:F1}/10 | Movimento: {(result.absoluteVelocityAlert ? "SIM" : "NÃO")}");
            }
        ...
        private void ProcessarZonasDeRiscoJob(float[] depthArrayManaged, ref MidasResult result)
        {
            ...
            // Aloca NativeArrays com base na resolução atual (currentTensorSize)
            ...
            var linhaEsq      = new NativeArray<float>(currentTensorSize, Allocator.TempJob);
            var linhaDir      = new NativeArray<float>(currentTensorSize, Allocator.TempJob);
            var linhaCen      = new NativeArray<float>(currentTensorSize, Allocator.TempJob);
            var countEsqArr   = new NativeArray<int>(currentTensorSize,   Allocator.TempJob);
            var countDirArr   = new NativeArray<int>(currentTensorSize,   Allocator.TempJob);
            var countCenArr   = new NativeArray<int>(currentTensorSize,   Allocator.TempJob);
            ...
            var zoneJob = new MidasZoneJob
            {
                ...
                tensorSize    = currentTensorSize,
                inverterEixoX = inverterEixoX ? 1 : 0,
                inverterProfundidade = inverterMapaProfundidade ? 1 : 0,
            ...
            JobHandle zoneHandle = zoneJob.Schedule(currentTensorSize, 4, maxHandle);
            zoneHandle.Complete();

            float somaE = 0f, somaD = 0f, somaC = 0f;
            int cE = 0, cD = 0, cC = 0;
            for (int y = 0; y < currentTensorSize; y++)
            {
